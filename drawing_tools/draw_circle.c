...
/*   Updated: 2024/01/12 17:38:44 by quteriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_filled_circle(t_canva *canva, t_point *center, int radius, int color)
{
	int	x;
	int	y;

	y = max(0, ceil(center->y - radius));
	while (y < min(floor(center->y + radius), WIN_HEIGHT))
	{
		x = max(0, ceil(center->x - radius));
		while (x < min(floor(center->x + radius), WIN_WIDTH))
		{
			if (inside_circle(center, radius, x, y))
